...
func NewFastMailProvider(token string) *FastMailProvider {
	return &FastMailProvider{Token: token}
}

func (f *FastMailProvider) GetJMAPSession() (*JMAPSession, error) {
...
func (f *FastMailProvider) GetIdentities(apiURL, accountID string) ([]Identity, error) {
...
func (f *FastMailProvider) ListAliases() ([]Identity, error) {
	session, err := f.GetJMAPSession()
	if err != nil {
		return nil, fmt.Errorf("failed to get JMAP session: %v", err)
	}
	...
	if primaryAccountID == "" {
		return nil, fmt.Errorf("no personal account found")
	}

	return f.GetIdentities(session.ApiURL, primaryAccountID)
}

func (f *FastMailProvider) TestConnection() error {
	...
	identities, err := f.GetIdentities(session.ApiURL, primaryAccountID)
	if err != nil {
...
// SyncAliasesToConfig converts FastMail identities to AccountConfig format
func (f *FastMailProvider) SyncAliasesToConfig(currentConfig interface{}, currentEmail string) ([]interface{}, error) {
	identities, err := f.ListAliases()
	if err != nil {
		return nil, err
	}
	...
	// Mark current email as existing
	existingEmails[currentEmail] = true

	// This would need to be adapted based on your actual AccountConfig type
	// For now, return as generic interfaces that can be converted later
	for _, identity := range identities {
		if strings.ToLower(identity.Email) == strings.ToLower(currentEmail) {
			continue
		}
	...
	fmt.Printf("✓ Ready to sync %d new aliases from FastMail\n", len(newAccounts))
	return newAccounts, nil
}

// GetTokenInstructions returns user-friendly instructions for getting a FastMail token
func GetTokenInstructions() string {
